...
\begin{enumerate}
    ...
    starting point has positive density $\pi(x_0) > 0$. Given the $\pi$-irreducibility
    of $K$ the chain will reach such a point by time $t$, or a swap will be
    accepted. In either case we can start our heuristic proof at this point.
...
Only RWM with step size $3.0$ is able to find all the modes, although it still
shows some tendency to get stuck in a mode for a while before breaking out.
...
rate is somewhat high, but it is still able to discover all modes for RWM,
although it missed a mode for MALA. On the other hand,
the corresponding unstabilized version is stuck in the initial mode in both cases.
% \item 
...
slightly different for RWM vs MALA. For RWM there appears to be a sweet spot near
$\sigma_\eta = 2^4$ where ESS peaks, and the rejection rate dips. This seems
reasonable as the reference is able to cover the modes without being too large.
However, MALA actually sees ESS peak near $\sigma_\eta = 2^{-2}$. This likely
corresponds to a local mode. It is also interesting that MALA doesn't see a
dip in the rejection rate around $\sigma_\eta = 2^2$ to $2^4$ like for RWM, even
though the swap rejection rate should in theory be the same for both samplers.
This may be due to the fact that we have not yet reached stationarity, but
it is not clear without further experiments.

In general the results are most conclusive for RWM, increasing $\sigma_\eta$
until it covers all the modes, but not beyond that should provide the best
reference. It is not clear why we don't  see the same benefit for MALA.
